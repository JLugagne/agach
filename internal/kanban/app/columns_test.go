...
func TestApp_UpdateColumnWIPLimit_Success(t *testing.T) {
	ctx := context.Background()
	...
	mockColumns.UpdateWIPLimitFunc = func(ctx context.Context, pid domain.ProjectID, cid domain.ColumnID, wipLimit int) error {
		calledWithLimit = wipLimit
		return nil
	}
	...
	require.NoError(t, err)
	assert.Equal(t, 5, calledWithLimit)
}

func TestApp_UpdateColumnWIPLimit_ColumnNotFound_ReturnsError(t *testing.T) {
	ctx := context.Background()
	a, _, _, _, mockColumns, _, _ := setupTestApp()
